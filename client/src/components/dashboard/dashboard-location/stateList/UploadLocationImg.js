...
const UploadLocationImg = () => {
  // redux
  const {errors, uploadedImage, uploadedImageChanged, oldImgUrl} = useSelector((state)=> state.state);
      ...
      <div className="profile-box position-relative d-md-flex align-items-end mb50">
        <div className="profile-img new position-relative overflow-hidden bdrs12 mb20-sm" style={{height: "100px", width: "100px"}}>
          <Image
            ...
            src={(!uploadedImageChanged && oldImgUrl ) ? `${process.env.NEXT_PUBLIC_SERVER_IMG_PATH}/assets/location/${oldImgUrl}` : uploadedImage || "/images/listings/profile-1.jpg"}
            alt="profile avatar"
          />
