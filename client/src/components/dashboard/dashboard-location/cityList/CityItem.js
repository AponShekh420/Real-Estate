...
const CityItem = ({eachCity, area, state}) => {
  const path = usePathname();
  const [loading, setLoading] = useState(false);
  const [deleteLoading, setDeleteLoading] = useState(false);

  // redux 
  const {active, areName, description, abbreviation, edit, stateId, areaId} = useSelector(state => state.city);
  ...
  const deactiveHanlder = async (e) => {
    try {
      ...
      if(dataRes.msg) {
        ...
        toast.error(`Please try again "${eachCity.name}" area`, {
          position: "top-right",
          autoClose: 1500,
        });
      }
