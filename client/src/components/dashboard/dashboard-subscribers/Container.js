...
const Container = () => {
  ...
  const getBlogsData = async () => {
    try {
      setLoading(true)
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API}/api/subscribe/getall`, {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      if(currentData.data) {
        ...
        setSubscribersData(currentData);
      } else {
          ...
          <div className={`ps-widget bgc-white bdrs12 default-box-shadow2 p30 mb30 overflow-hidden position-relative ${loading ? "d-flex justify-content-center align-items-center": ""}`}>
            <div className={`packages_table table-responsive ${loading ? "d-flex justify-content-center align-items-center": ""}`} style={loading ? {height:"400px"} : {}}>
              {loading ? (
                ...
                ) : subscribersData?.data?.length == 0 ? <h1 style={{height: "400px"}} className="d-flex align-items-center justify-content-center">No Data Found</h1> : <SubscribersTable setDeleteData={setDeleteData} subscribersData={subscribersData} key={2}/>}

              <div className="mt30">
                {loading ? (<div></div>) : (<Pagination currentPage={currentPage} setCurrentPage={setCurrentPage} totalPages={totalPages} data={subscribersData}/>)}
              </div>
            </div>
          </div>
        </div>
