  ...
  try {
    if (slug?.length == 1 && slug !== undefined) {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/state/get-by-slug`,
        {
          method: "POST",
          cache: "no-store",
          credentials: "include",
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/area/get-by-slug`,
        {
          method: "POST",
          cache: "no-store",
          credentials: "include",
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/city/get-by-slug`,
        {
          method: "POST",
          cache: "no-store",
          credentials: "include",
