...
const uploadModelImg = (req, res, next) => {
  const upload = uploader(
    ...
    ["image/jpeg", "image/jpg", "image/png", "application/pdf"], // Allowed mime types
    20971520, // Max file size (10MB in this case)
    "Only jpg, jpeg,png and pdf allowed" // Error message for invalid file types
  );
  ...
  upload.single("img")(req, res, async (err) => {
    ...
    try {
      existingImage = req.body.existingImage
        ? JSON.parse(req.body.existingImage)
        : null;
    } catch (error) {
    ...
    try {
      deletedImage = req.body.deletedImage
        ? JSON.parse(req.body.deletedImage)
        : null;
    } catch (error) {
